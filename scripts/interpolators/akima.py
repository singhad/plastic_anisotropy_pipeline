...
def interpolate_property_akima(
    df_clean: pd.DataFrame,
    property_name: str,
    grid_size: int = 100
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Akima-like surface via SciPy's cubic 'griddata' (piecewise cubic on Delaunay).
    Fills any NaNs (outside convex hull) with nearest-neighbour values.
    """
    ...
    if np.isnan(grid_z).any():
        # Fill outside the convex hull with nearest
        grid_z_nn = griddata(pts, z, (grid_x, grid_y), method="nearest")
...
def _proximity_sigma_grid(
    ...
    """
    Simple distance-to-data uncertainty:
...
def _fit_and_score_akima(df_clean: pd.DataFrame, property_name: str) -> Tuple[float, float, float]:
    """
    In-sample metrics at sample points using the same cubic griddata interpolant.
    """
    x = df_clean["Time"].to_numpy()
    y = df_clean["Temperature"].to_numpy()
    z = df_clean[property_name].to_numpy()
    ...
    zhat = griddata(pts, z, pts, method="cubic")
    # Any rare NaNs (degenerate triangulation): backfill with nearest
    if np.isnan(zhat).any():
        zhat_nn = griddata(pts, z, pts, method="nearest")
        nan_mask = np.isnan(zhat)
        zhat[nan_mask] = zhat_nn[nan_mask]
...
def execute_akima_pipeline(
    ...
    """
    Run Akima-like (cubic griddata) interpolation for each property and save:
      - grids:  outputs/akima/grids/<prop>_akima_grid.csv
      - plots:  plots/akima/plots/<prop>_akima_plot.png
    Optional:
      - proximity sigma: <prop>_sigma_akima_grid.csv + plot
    ...
    logger.info("AKIMA-like interpolation pipeline started.")
    ...
    try:
        ...
        for prop in properties:
            ...
            gx, gy, gz = interpolate_property_akima(df_clean, prop, grid_size=grid_size)
            ...
            if benchmark:
                rmse, mae, r2 = _fit_and_score_akima(df_clean, prop)
                metrics_rows.append({
                    "technique": technique,
                    "property": prop,
                    "grid_size": grid_size,
        ...
        logger.info("AKIMA-like pipeline completed.")

    except Exception as e:
        logger.exception(f"Akima pipeline failed: {e}")
        raise
