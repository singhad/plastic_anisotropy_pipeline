...
# Note: this uses Clough–Tocher (piecewise cubic, C¹) which is often called 
# “biharmonic” in practice, but it’s not a true biharmonic spline.

# ---------------- core interpolation ----------------

def interpolate_property_biharmonic(
    df_clean: pd.DataFrame,
    property_name: str,
    grid_size: int = 100
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Smooth C¹ surface via Clough–Tocher (piecewise cubic on Delaunay).
...
def _fit_and_score_biharmonic(
    df_clean: pd.DataFrame,
    property_name: str
) -> Tuple[float, float, float]:
    """
    In-sample metrics at sample points using the same Clough–Tocher interpolant,
...
def execute_biharmonic_pipeline(
    ...
    """
    Run Clough–Tocher ('biharmonic-like') interpolation for each property and save:
      - grids:  outputs/biharmonic/grids/<prop>_biharmonic_grid.csv
      - plots:  plots/biharmonic/plots/<prop>_biharmonic_plot.png
    Optional:
      - proximity sigma: <prop>_sigma_biharmonic_grid.csv + plot
      - in-sample metrics: RMSE/MAE/R2 (logged)
    """
    base_path = get_base_path()
    setup_repo_structure(base_path)

    technique = "biharmonic"
    ...
    logger.info("BIHARMONIC (Clough–Tocher) interpolation pipeline started.")
    ...
    try:
        ...
        for prop in properties:
            ...
            gx, gy, gz = interpolate_property_biharmonic(df_clean, prop, grid_size=grid_size)
            ...
            if benchmark:
                rmse, mae, r2 = _fit_and_score_biharmonic(df_clean, prop)
                metrics_rows.append({
                    "technique": technique,
                    "property": prop,
                    "grid_size": grid_size,
        ...
        logger.info("BIHARMONIC pipeline completed.")

    except Exception as e:
        logger.exception(f"Biharmonic pipeline failed: {e}")
        raise
